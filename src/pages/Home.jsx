...
const Home = () => {
  ...
  const submitRSVP = async () => {
    ...
    for (const g of guestList) { if (g.asistencia === 'si') { algunAsistente = true; if (!g.starter || !g.entree) { setAlertModal({ open: true, message: `Por favor selecciona la entrada y plato fuerte para ${g.nombre}.` }); setLoading(false); window.scrollTo(0, 0); return; } } }
  ...
  return (
    <Box>
      <Box sx={{ height: '50vh', backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.4)), url('/hampton_court.webp')`, backgroundSize: 'cover', backgroundPosition: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', color: 'white', overflow: 'hidden' }}>
        {/* CORRECCIÓN UX: Aumento del valor mínimo del clamp a 4.5rem para que tenga más peso en celulares pequeños. */}
        <Typography variant="h1" color="white" sx={{ fontSize: 'clamp(4.5rem, 12vw, 9rem)', lineHeight: 0.9, mt: 4, textAlign: 'center' }}>Shields Urquilla</Typography>
        <Typography sx={{ letterSpacing: 3, mt: 3, fontSize: { xs: '0.8rem', sm: '1rem' }, textAlign: 'center' }}>CELEBRACIÓN DE COMPROMISO</Typography>
        <Typography variant="h4" sx={{ mt: 1, color: 'secondary.main', fontSize: { xs: '1rem', sm: '1.2rem' }, textAlign: 'center' }}>15 de Noviembre de 2026</Typography>
      </Box>
              ...
              {guestList.map((guest, index) => (
                <Box key={index} sx={{ border: '1px dashed #711c2e', borderRadius: 2, p: { xs: 2, md: 3 }, mb: 3 }}>
                  ...
                  <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2, mb: guest.asistencia === 'si' ? 3 : 0 }}>
                    <FormControl fullWidth sx={{ width: '100%' }}><InputLabel>¿Asistirá?</InputLabel><Select fullWidth value={guest.asistencia} label="¿Asistirá?" onChange={(e) => handleGuestChange(index, 'asistencia', e.target.value)}><MenuItem value="si">Sí asistirá</MenuItem><MenuItem value="no">No asistirá</MenuItem></Select></FormControl>
                    {guest.asistencia === 'si' && (<FormControl fullWidth sx={{ width: '100%' }}><InputLabel>Restricciones Alimenticias</InputLabel><Select fullWidth value={guest.dieta} label="Restricciones Alimenticias" onChange={(e) => handleGuestChange(index, 'dieta', e.target.value)}><MenuItem value="None">Ninguna</MenuItem><MenuItem value="Gluten free">Gluten free</MenuItem><MenuItem value="Vegetarian">Vegetariano</MenuItem><MenuItem value="Vegan">Vegano</MenuItem></Select></FormControl>)}
                  </Box>
                  {guest.asistencia === 'si' && (
                    <Box>
                      {guest.tipo === 'niño' ? (
                        ...
                        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
                          <FormControl fullWidth sx={{ width: '100%' }}><InputLabel>Entradas</InputLabel><Select fullWidth value={guest.starter || ''} label="Entradas" onChange={(e) => handleGuestChange(index, 'starter', e.target.value)}><MenuItem value="Caesar Salad">"Prince of Wales" Caesar Salad</MenuItem><MenuItem value="Tomato Soup">Tomato Soup</MenuItem></Select></FormControl>
                          <FormControl fullWidth sx={{ width: '100%' }}><InputLabel>Plato Fuerte</InputLabel><Select fullWidth value={guest.entree || ''} label="Plato Fuerte" onChange={(e) => handleGuestChange(index, 'entree', e.target.value)}><MenuItem value="Grilled Chicken Breast">Grilled Chicken Breast</MenuItem><MenuItem value="Three Cheese Tortellini">Three Cheese Tortellini</MenuItem></Select></FormControl>
                        </Box>
                      )}
                    </Box>
                  )}
